...
function startNewGame() {
    ...
    // Shuffle cards
    var shuffledIndexes = shuffleIndexes();

    //  Create card objects
    createStackOfCards(shuffledIndexes);
...
function createStackOfCards(shuffledIndexes) {
    ...
    if (component.status === Component.Ready) {
        ...
        for (var i = 0; i < shuffledIndexes.length; i++) {
            ...
            var cardProperties = cardsInfo[shuffledIndexes[i]];
            var card = component.createObject(gameArea.stackOfCardsArea,
                                              {cardObject: cardProperties});
...
function shuffleIndexes() {
    ...
    var array = Array.apply(null, {length: cardsInfo.length}).map(Number.call, Number);
